...
//        self.addConstraint(NSLayoutConstraint.init(item: y, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 1))
        ...
        for i in 0 ..< 10 {
            for j in 0 ..< 20 {
                ...
                let unit = CCColorUnit(radius: radius * ratio[i])
                unit.center = CGPoint(x: centerXY, y: centerXY)
                addSubview(unit)
                var x: CGFloat
                var y: CGFloat
                var angle: CGFloat
                ...
                var blue: CGFloat = 1
                if i == 0 {
                    unit.backgroundColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1)
                    continue
                } else if i == 1 {
                ...
                unit.backgroundColor = UIColor(red: red, green: green, blue: blue, alpha: unitAlpha)
            }
        }
    }
}
